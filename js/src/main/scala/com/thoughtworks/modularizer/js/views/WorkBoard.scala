...
class WorkBoard(val branch: String)(implicit fetcher: GlobalFetch,
  ...
  def board(graph: Graph, initialRule: ClusteringRule, initialETag: Option[String]): Binding[BindingSeq[Node]] = {
    ...
    val summaryDiagram = new SummaryDiagram(breakingEdges, rule, clusteringService)
    ...
    <div class="d-flex flex-row flex-grow-1" style:minHeight="0">
      { DependencyExplorer.render(graph, draftClusters, clusteringService, rule, ruleEditor.selectedNodeIds).bind }
              ...
              <button
                ...
                onclick={
                  clusteringService.underlyingCompoundGraph.bind match {
                    case None => _: Event =>
                    case Some(compoundGraph) => _: Event =>
